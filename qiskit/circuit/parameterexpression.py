...
class ParameterExpression():
    ...
    def subs(self, parameter_map):
        ...
        # KDK Can we take these directly from inbound_parameter._parameter_symbols?
        from sympy import Symbol
        new_parameter_symbols = {p: Symbol(p.name)
                                 for p in inbound_parameters}

        # KDK Same
        # KDK Add note a la N.B. This works only because Symbols' only identiy is their names, so we reuse freely, unlike our Parameters
